...
std::string obtener_directorio_ejecutable() {
	...
	if (longitud <= 0) {
		std::cerr << "Aviso: no se pudo resolver /proc/self/exe; se usará el directorio actual para la configuración." << std::endl;
		return ".";
	}
	buffer[longitud] = '\0';
...
AppConfig cargar_configuracion() {
	...
	while (std::getline(archivo, linea)) {
		...
		try {
			...
			std::cerr << "Aviso: valor inválido para '" << clave << "' en " << ruta << " (" << e.what() << "). Se usará el valor por defecto." << std::endl;
		}
	}
	return config;
}
