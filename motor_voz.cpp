...
std::vector<short> convertir_a_canales(const std::vector<short> &in, int canales) {
	if (canales <= 1) {
		return in;
	}

	std::vector<short> out;
	out.reserve(in.size() * static_cast<size_t>(canales));
...
int al_recibir_muestras_sintesis(short *muestras, int cantidad, espeak_EVENT *eventos) {
	...
	if (muestras == nullptr || cantidad <= 0) {
		return 0;
	}

	{
		...
		if (g_buffer_exportacion != nullptr) {
...
void hilo_playback_portaudio() {
    ...
    while (g_playback_thread_running) {
        ...
        if (!chunk.empty()) {
            ...
            PaError err = Pa_WriteStream(g_pa_stream, out.data(), static_cast<unsigned long>(out.size() / static_cast<size_t>(g_channels)));
...
bool inicializar_espeak(int modo_salida) {
	int resultado = espeak_Initialize(static_cast<espeak_AUDIO_OUTPUT>(modo_salida), 0, nullptr, 0);
...
bool inicializar() {
	return inicializar_espeak(AUDIO_OUTPUT_PLAYBACK);
}
...
void finalizar() {
    ...
    if (g_playback_thread_running) {
        ...
        if (g_playback_thread.joinable()) {
            g_playback_thread.join();
        }
    }
...
std::vector<VozInfo> listar_voces_disponibles() {
	...
	const espeak_VOICE **voces_espeak = espeak_ListVoices(nullptr);
...
void configurar_desde_appconfig(const Configuracion::AppConfig &config) {
	...
	if (g_audio_backend == "portaudio") {
        ...
        int resultado = espeak_Initialize(AUDIO_OUTPUT_RETRIEVAL, 0, nullptr, 0);
        ...
        if (err != paNoError) {
            ...
            if (outParams.device == paNoDevice) {
                ...
                err = Pa_OpenStream(&g_pa_stream, nullptr, &outParams, static_cast<double>(g_sample_rate), g_audio_buffer_frames, paNoFlag, nullptr, nullptr);
...
#else
		std::cerr << "PortAudio no está disponible en esta compilación. Recompile con HAVE_PORTAUDIO definido y libportaudio instalada." << std::endl;
		g_audio_backend = "espeak_playback";
#endif
	} else {
		espeak_Terminate();

		int resultado = espeak_Initialize(AUDIO_OUTPUT_PLAYBACK, 0, nullptr, 0);
...
void hablar(const std::string &texto, const CallbackEstado &callback_estado) {
	...
	int resultado = espeak_Synth(texto.c_str(), texto.length() + 1, 0, POS_CHARACTER, 0, flags, nullptr, nullptr);
...
bool exportar_a_wav(const std::string &texto, const std::string &ruta_salida, const CallbackEstado &callback_estado) {
	...
	if (!inicializar_espeak(AUDIO_OUTPUT_RETRIEVAL)) {
		std::lock_guard<std::mutex> guardia(g_mutex_exportacion);
		g_buffer_exportacion = nullptr;
		g_hablando = false;
		return false;
	...
	if (resultado == EE_OK) {
		...
		if (muestras.empty()) {
			std::cerr << "Error: eSpeak no entregó muestras PCM." << std::endl;

			if (callback_estado) {
				callback_estado("❌ La síntesis no produjo muestras de audio.", true);
			}
		} else {
			std::vector<short> salida = convertir_a_canales(muestras, g_channels);
			exito = escribir_wav(ruta_salida, salida, g_sample_rate, g_channels);

			if (callback_estado) {
				callback_estado(exito ? "✅ Archivo WAV exportado correctamente." : "❌ Error al escribir el archivo WAV.", true);
			}
		}
	} else {
		std::lock_guard<std::mutex> guardia(g_mutex_exportacion);
	...
	if (!inicializar_espeak(AUDIO_OUTPUT_PLAYBACK)) {
		std::cerr << "Aviso: no se pudo restaurar AUDIO_OUTPUT_PLAYBACK." << std::endl;
	}
...
void leer_mensaje_estado(const std::string &mensaje) {
	...
	espeak_Synth(mensaje_limpio.c_str(), mensaje_limpio.length() + 1, 0, POS_CHARACTER, 0, espeakCHARS_AUTO, nullptr, nullptr);
	espeak_Synchronize();
}
